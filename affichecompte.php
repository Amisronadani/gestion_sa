...
if (isset($_GET['id']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("DELETE FROM compte WHERE numero_compte = ?");
...
$stmt = $pdo->query("SELECT * FROM compte c join client cl on c.id_client=cl.id_client LIMIT $limit OFFSET $offset");
$compte = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Récupérer le nombre total d'éléments
$total_query = "SELECT COUNT(*) FROM compte";
...
<head>
    <meta charset="UTF-8">
    <title>Liste des Compte du Client</title>
...
?>
     ...
     <h1>Gestion des <span>Comptes</span></></h1>
    <a href="Compte.php" class="ajouter">➕ Creer une compte</a>
        ...
        <tbody id="clientList">
            <?php foreach ($compte as $compte): ?>
                <tr>
                    ...
                    <td><?= $compte['solde'] ?> Fbu</td>
                    <td><?= $compte['decouvert'] ?></td>
                    <td><?= $compte['statut'] ?></td>
                    <td>
                        <a href="modifiercompte.php?id=<?= $compte['numero_compte'] ?>" class="modifier"> ✏️ Modifier</a>
                    </td>
                    <td>
                    <form action="?id=<?= $compte['numero_compte'] ?>" method="POST" style="display:inline;">
